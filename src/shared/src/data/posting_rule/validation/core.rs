...
impl PostingRuleValidator {
    pub async fn validate(posting_rule: &PostingRule) -> Vec<String> {
        let mut issues = vec![];

        BasePostingRuleValidator::validate(posting_rule.base(), &mut issues).await;

        match posting_rule {
            PostingRule::Text(text_rule) => {
                TextPostingRuleValidator::validate(text_rule, &mut issues).await;
            }
            PostingRule::Poll(poll_rule) => {
                PollPostingRuleValidator::validate(poll_rule, &mut issues).await;
            }
        }
